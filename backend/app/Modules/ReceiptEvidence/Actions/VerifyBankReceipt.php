...
final class VerifyBankReceipt
{
    ...
    public function execute(string $tenantId, User $actor, array $input): string
    {
        ...
        return $this->tx->run(function () use ($tenantId, $actor, $facts): string {
            ...
            if ((string) $account->valid_from > $facts['control_date'] || ($account->retired_from !== null && $facts['control_date'] >= (string) $account->retired_from)) {
                throw new ReceiptEvidenceConflict('Receiving account was not approved at receipt control date.');
            }
            ...
            if ($byOperation !== null) {
                if ($this->matches($byOperation, $facts)) {
            ...
            if ($bySource !== null) {
                if ($this->matches($bySource, $facts)) {
                    return (string) $bySource->id;
                }
            ...
            if ($facts['replaces_receipt_id'] !== null) {
                ...
                if ($original === null || $original->status !== 'invalidated') {
                    throw new ReceiptEvidenceConflict('Replacement receipt requires an invalidated original receipt.');
                }
            }
            $id = (string) Str::ulid();
            $now = now();
    ...
    {
        $account = (string) ($input['receiving_account_id'] ?? '');
        $kind = (string) ($input['source_identity_kind'] ?? '');
        $source = trim((string) ($input['source_identity'] ?? ''));
        $reference = trim((string) ($input['evidence_reference'] ?? ''));
        if ($account === '' || ! in_array($kind, ['bank_transaction_id', 'statement_line_fingerprint_v1'], true) || $source === '' || $reference === '') {
            throw new ReceiptEvidenceValidationFailed('receiving_account_id, supported source identity, and evidence_reference are required.');
        }
        $version = (int) ($input['source_identity_version'] ?? 0);
        if ($version < 1) {
            throw new ReceiptEvidenceValidationFailed('source_identity_version must be positive.');
        }
