    ...
    public function execute(string $tenantId, string $accountId, User $actor, array $input): void
    {
        ...
        $this->tx->run(function () use ($tenantId, $accountId, $actor, $facts): void {
            ...
            if ($account->status === 'retired') {
                if ($account->retirement_operation_id === $facts['retirement_operation_id'] && (string) $account->retired_from === $facts['retired_from'] && $account->retirement_reason === $facts['retirement_reason']) {
                    return;
                }
                throw new ReceiptEvidenceConflict('Receiving account is already retired by a different terminal operation.');
            }
            if ($facts['retired_from'] <= (string) $account->valid_from) {
                throw new ReceiptEvidenceConflict('retired_from must be after valid_from.');
            }
            ...
            if ($operation !== null) {
                throw new ReceiptEvidenceConflict('Retirement operation identity was reused with different facts.');
            }
            $now = now();
