...
final class AssociateReceiptWithPayment
{
    ...
    public function execute(string $tenantId, User $actor, array $input): string
    {
        ...
        return $this->tx->run(function () use ($tenantId, $actor, $facts): string {
            ...
            if ($payment === null) throw (new ModelNotFoundException)->setModel('Payment');
            ...
            if ($byReceipt !== null) {
                if ($this->matches($byReceipt, $facts)) return (string) $byReceipt->id;
            ...
            if ($byPayment !== null) throw new ReceiptEvidenceConflict('Payment already has an effective receipt association.');
            if ($receipt->status !== 'effective' || $payment->status !== 'received' || (string) $receipt->amount !== (string) $payment->amount || $receipt->currency !== $payment->currency) throw new ReceiptEvidenceConflict('Receipt and Payment are not eligible for exact association.');
            if ($facts['replaces_association_id'] !== null) {
                ...
                if ($original === null || $original->status !== 'cancelled') throw new ReceiptEvidenceConflict('Replacement association requires a cancelled original.');
            }
    ...
    {
        $receipt = (string) ($input['receipt_id'] ?? ''); $payment = (string) ($input['payment_id'] ?? '');
        if ($receipt === '' || $payment === '') throw new ReceiptEvidenceValidationFailed('receipt_id and payment_id are required.');
