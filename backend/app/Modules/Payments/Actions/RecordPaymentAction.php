...
{
    public function __construct(private readonly PaymentsTransaction $tx, private readonly ReceivablesAuthorization $auth) {}
    ...
    private function canonicalize(array $input): array
    {
        ...
        if (! Str::isUlid($operation)) throw new PaymentsValidationFailed('payment_operation_id must be a caller-supplied ULID.');
        $currency = (string) ($input['currency'] ?? '');
        if (! in_array($currency, ['SAR', 'USD'], true)) throw new PaymentsValidationFailed('currency must be SAR or USD.');
        $receivedOn = (string) ($input['received_on'] ?? '');
        $date = CarbonImmutable::createFromFormat('!Y-m-d', $receivedOn, 'UTC');
        if ($date === false || $date->format('Y-m-d') !== $receivedOn || $date->isFuture()) throw new PaymentsValidationFailed('received_on must be a non-future YYYY-MM-DD date.');
        $customer = (string) ($input['customer_id'] ?? '');
        if ($customer === '') throw new PaymentsValidationFailed('customer_id is required.');
