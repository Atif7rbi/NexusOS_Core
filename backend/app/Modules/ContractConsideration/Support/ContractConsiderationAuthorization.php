...
final class ContractConsiderationAuthorization
{
    ...
    public function lock(string $tenantId, User $actor): void
    {
        try {
            $this->authorization->authorizeTransactionalTenantAdministrator($tenantId, $actor);
        } catch (ReceivablesAccessDenied $exception) {
            throw new ContractConsiderationAccessDenied('Actor is not authorized for Contract Consideration.', previous: $exception);
        }
    }
}
