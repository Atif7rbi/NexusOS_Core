...
final class ActivateAccountingAction
{
    ...
    public function execute(string $tenantId, User $actor): string
    {
        ...
        return $this->tx->run(function () use ($tenantId, $actor): string {
            ...
            if ($tenant === null || $tenant->status !== Tenant::STATUS_ACTIVE || $tenant->currency !== 'SAR') { throw new AccountingValidationFailed('Accounting activation requires an active SAR Tenant.'); }
