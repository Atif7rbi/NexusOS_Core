...
final class ManageAccountAction
{
    ...
    public function create(string $tenantId, User $actor, array $data): string
    {
        ...
        return $this->tx->run(function () use ($tenantId, $actor, $data): string {
            ...
            $this->audit->write($tenantId,'account.created','account',$id,(int)$actor->id,[],$at); return $id;
        });
    }
    public function update(string $tenantId, string $accountId, User $actor, array $changes): void
    {
        ...
        $this->tx->run(function () use($tenantId,$accountId,$actor,$changes): void {
            ...
            if($account===null){throw new AccountingValidationFailed('Account was not found.');}
    ...
    public function archive(string $tenantId,string $accountId,User $actor): void { $this->lifecycle($tenantId,$accountId,$actor,true); }
    public function restore(string $tenantId,string $accountId,User $actor): void { $this->lifecycle($tenantId,$accountId,$actor,false); }
    private function lifecycle(string $tenantId,string $accountId,User $actor,bool $archive): void
    {
        ...
        $this->tx->run(function()use($tenantId,$accountId,$actor,$archive):void{
            $at=now(); DB::table('accounts')->where('tenant_id',$tenantId)->where('id',$accountId)->lockForUpdate()->first() ?? throw new AccountingValidationFailed('Account was not found.');
