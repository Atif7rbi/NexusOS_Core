...
final class ReverseJournalAction
{
    ...
    {
        $this->auth->authorize($tenantId,$actor,'reverse_journal');if(trim($reason)===''){throw new AccountingValidationFailed('Reversal reason is required.');}
        return $this->tx->run(function()use($tenantId,$targetId,$actor,$entryDate,$reason):PostedJournalResult{
            $opening=$this->findOpening($tenantId,$targetId);if($opening!==null){DB::table('accounting_settings')->where('tenant_id',$tenantId)->lockForUpdate()->first();}
            $target=DB::table('journal_entries')->where('tenant_id',$tenantId)->where('id',$targetId)->lockForUpdate()->first();
            if($target===null||$target->status!=='posted'||$entryDate<$target->entry_date){throw new AccountingValidationFailed('Only a terminal Posted Journal can be reversed chronologically.');}
            if(DB::table('journal_entries')->where('tenant_id',$tenantId)->where('reverses_journal_entry_id',$targetId)->exists()){throw new AccountingConflict('Journal already has a direct reversal.');}
            ...
            $id=(string)Str::ulid();$at=now();DB::table('journal_entries')->insert(['id'=>$id,'tenant_id'=>$tenantId,'entry_date'=>$entryDate,'description'=>'Reversal: '.$target->description,'status'=>'draft','origin'=>'reversal','source_type'=>'journal_entry','source_id'=>$targetId,'created_by'=>$actor->id,'updated_by'=>$actor->id,'created_at'=>$at,'updated_at'=>$at,'reverses_journal_entry_id'=>$targetId,'reversal_reason'=>$reason]);
            foreach($lines as $line){DB::table('journal_lines')->insert(['id'=>(string)Str::ulid(),'tenant_id'=>$tenantId,'journal_entry_id'=>$id,'line_number'=>$line->line_number,'account_id'=>$line->account_id,'debit'=>$line->credit,'credit'=>$line->debit,'memo'=>$line->memo,'created_at'=>$at,'updated_at'=>$at]);}
            $result=$this->posting->post($tenantId,$id,$actor);$this->audit->write($tenantId,'journal.reversed','journal_entry',$targetId,(int)$actor->id,['reversal_journal_entry_id'=>$id,'reason'=>$reason],$at);
            if($opening!==null){$state=$opening->effect_state==='effective'?'neutralized':'effective';DB::table('opening_balance_operations')->where('tenant_id',$tenantId)->where('id',$opening->id)->lockForUpdate()->update(['effect_state'=>$state,'latest_effect_journal_entry_id'=>$id,'effect_updated_by'=>$actor->id,'effect_updated_at'=>$at,'updated_by'=>$actor->id,'updated_at'=>$at]);$this->audit->write($tenantId,$state==='effective'?'opening_balance.reactivated':'opening_balance.reversed','opening_balance_operation',$opening->id,(int)$actor->id,['journal_entry_id'=>$id],$at);}
            return $result;
        });
    }
