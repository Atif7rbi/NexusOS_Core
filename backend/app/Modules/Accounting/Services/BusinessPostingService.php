...
final class BusinessPostingService implements BusinessPostingServiceInterface
{
    ...
    {
        if(DB::transactionLevel()<1){throw new \LogicException('Business posting requires a caller-owned transaction.');}
        ...
        if($existing!==null){return $this->replay($existing,$request);}
        if(!DB::table('accounting_source_types')->where('origin','business')->where('key',$request->sourceType)->exists()){throw new AccountingValidationFailed('Business source type is not registered.');}
        $id=(string)Str::ulid();$at=now();
        ...
        foreach(array_values($request->lines) as $i=>$line){DB::table('journal_lines')->insert(['id'=>(string)Str::ulid(),'tenant_id'=>$request->tenantId,'journal_entry_id'=>$id,'line_number'=>$i+1,'account_id'=>$line->accountId,'debit'=>(string)$line->debit,'credit'=>(string)$line->credit,'memo'=>$line->memo,'created_at'=>$at,'updated_at'=>$at]);}
    ...
    private function replay(object $existing,BusinessPostingRequest $request):PostedJournalResult
    {
        ...
        foreach($request->lines as $i=>$expected){$actual=$lines[$i]??null;$same=$same&&$actual!==null&&$actual->account_id===$expected->accountId&&(string)$actual->debit===(string)$expected->debit&&(string)$actual->credit===(string)$expected->credit&&$actual->memo===$expected->memo;}
        if(!$same){throw new AccountingConflict('Business source identity was reused with different accounting facts.');}
