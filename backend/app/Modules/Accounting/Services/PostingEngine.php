...
final class PostingEngine
{
    ...
    {
        if (DB::transactionLevel() < 1) { throw new \LogicException('PostingEngine requires a caller-owned transaction.'); }
        $journal = DB::table('journal_entries')->where('tenant_id', $tenantId)->where('id', $journalId)->lockForUpdate()->first();
        if ($journal === null) { throw new AccountingValidationFailed('Journal was not found.'); }
        ...
        if ($period === null || $period->status !== 'open') { throw new AccountingValidationFailed('Journal requires a containing open period.'); }
        ...
        if ($lines->count() < 2) { throw new AccountingValidationFailed('Journal requires at least two lines.'); }
        ...
        if ($accounts->count() !== count($accountIds)) { throw new AccountingValidationFailed('A journal account was not found.'); }
        ...
        foreach ($lines as $index => $line) {
            if ((int) $line->line_number !== $index + 1) { throw new AccountingValidationFailed('Journal lines must be contiguous.'); }
            $debit = $debit->plus(Money::of((string) $line->debit));
            $credit = $credit->plus(Money::of((string) $line->credit));
        }
        if ($debit->isZero() || ! $debit->equals($credit)) { throw new AccountingValidationFailed('Journal must be non-zero and balanced.'); }
        ...
        if ($updated !== 1) { throw new AccountingConflict('Journal changed while posting.'); }
