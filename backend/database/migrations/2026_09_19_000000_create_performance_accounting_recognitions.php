        ...
        DB::unprepared(<<<'SQL'
            ...
            BEGIN
              ...
              ELSE
                ...
                IF EXISTS (
                  SELECT 1
                  FROM public.accounting_position_consumptions pc
                  WHERE pc.tenant_id=r.tenant_id
                    AND pc.consuming_recognition_type='PERFORMANCE_ACCOUNTING_RECOGNITION'
                    AND pc.consuming_recognition_id=predecessor.id
                    AND NOT EXISTS (
                      SELECT 1
                      FROM public.accounting_position_consumptions sc
                      WHERE sc.tenant_id=r.tenant_id
                        AND sc.consuming_recognition_type='PERFORMANCE_ACCOUNTING_RECOGNITION'
                        AND sc.consuming_recognition_id=r.id
                        AND sc.origin_id=pc.origin_id
                        AND sc.consideration_transition_id=pc.consideration_transition_id
                        AND sc.consideration_lot_id=pc.consideration_lot_id
                        AND sc.economic_leg_identity=pc.economic_leg_identity
                        AND sc.amount=pc.amount
                        AND sc.currency=pc.currency
                    )
                )
                OR EXISTS (
                  SELECT 1
                  ...
                  WHERE sc.tenant_id=r.tenant_id
                    ...
                    AND NOT EXISTS (
                      ...
                      WHERE pc.tenant_id=r.tenant_id
                        AND pc.consuming_recognition_type='PERFORMANCE_ACCOUNTING_RECOGNITION'
                        AND pc.consuming_recognition_id=predecessor.id
                        AND pc.origin_id=sc.origin_id
                        AND pc.consideration_transition_id=sc.consideration_transition_id
                        AND pc.consideration_lot_id=sc.consideration_lot_id
                        AND pc.economic_leg_identity=sc.economic_leg_identity
                        AND pc.amount=sc.amount
                        AND pc.currency=sc.currency
                    )
                ) THEN
                  RAISE EXCEPTION USING ERRCODE='23514',
                ...
                IF EXISTS (
                  SELECT 1
                  FROM public.accounting_position_origins po
                  WHERE po.tenant_id=r.tenant_id
                    AND po.origin_recognition_type='PERFORMANCE_ACCOUNTING_RECOGNITION'
                    AND po.origin_recognition_id=predecessor.id
                    AND NOT EXISTS (
                      SELECT 1
                      FROM public.accounting_position_origins so
                      WHERE so.tenant_id=r.tenant_id
                        AND so.origin_recognition_type='PERFORMANCE_ACCOUNTING_RECOGNITION'
                        AND so.origin_recognition_id=r.id
                        AND so.consideration_transition_id=po.consideration_transition_id
                        AND so.consideration_lot_id=po.consideration_lot_id
                        AND so.economic_leg_identity=po.economic_leg_identity
                        AND so.origin_amount=po.origin_amount
                        AND so.currency=po.currency
                        AND so.accounting_date=po.accounting_date
                    )
                )
                OR EXISTS (
                  SELECT 1
                  ...
                  WHERE so.tenant_id=r.tenant_id
                    ...
                    AND NOT EXISTS (
                      ...
                      WHERE po.tenant_id=r.tenant_id
                        AND po.origin_recognition_type='PERFORMANCE_ACCOUNTING_RECOGNITION'
                        AND po.origin_recognition_id=predecessor.id
                        AND po.consideration_transition_id=so.consideration_transition_id
                        AND po.consideration_lot_id=so.consideration_lot_id
                        AND po.economic_leg_identity=so.economic_leg_identity
                        AND po.origin_amount=so.origin_amount
                        AND po.currency=so.currency
                        AND po.accounting_date=so.accounting_date
                    )
                ) THEN
                  RAISE EXCEPTION USING ERRCODE='23514',
