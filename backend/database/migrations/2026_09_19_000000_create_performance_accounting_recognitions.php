            ...
            BEGIN
              ...
              ELSE
                ...
                IF predecessor.id IS NULL
                   ...
                   OR predecessor.currency<>r.currency
                   OR predecessor.reversal_operation_id<>r.performance_accounting_correction_operation_id
                THEN
                ...
                IF EXISTS (
                  SELECT 1
                  FROM public.accounting_position_consumptions predecessor_consumption
                  WHERE predecessor_consumption.tenant_id=r.tenant_id
                    AND predecessor_consumption.consuming_recognition_type='PERFORMANCE_ACCOUNTING_RECOGNITION'
                    AND predecessor_consumption.consuming_recognition_id=predecessor.id
                    AND NOT EXISTS (
                      SELECT 1
                      FROM public.accounting_position_consumptions successor_consumption
                      WHERE successor_consumption.tenant_id=r.tenant_id
                        AND successor_consumption.consuming_recognition_type='PERFORMANCE_ACCOUNTING_RECOGNITION'
                        AND successor_consumption.consuming_recognition_id=r.id
                        AND successor_consumption.origin_id=predecessor_consumption.origin_id
                        AND successor_consumption.consideration_transition_id=predecessor_consumption.consideration_transition_id
                        AND successor_consumption.consideration_lot_id=predecessor_consumption.consideration_lot_id
                        AND successor_consumption.economic_leg_identity=predecessor_consumption.economic_leg_identity
                        AND successor_consumption.amount=predecessor_consumption.amount
                        AND successor_consumption.currency=predecessor_consumption.currency
                    )
                )
                OR EXISTS (
                  SELECT 1
                  ...
                  WHERE successor_consumption.tenant_id=r.tenant_id
                    ...
                    AND NOT EXISTS (
                      ...
                      WHERE predecessor_consumption.tenant_id=r.tenant_id
                        AND predecessor_consumption.consuming_recognition_type='PERFORMANCE_ACCOUNTING_RECOGNITION'
                        AND predecessor_consumption.consuming_recognition_id=predecessor.id
                        AND predecessor_consumption.origin_id=successor_consumption.origin_id
                        AND predecessor_consumption.consideration_transition_id=successor_consumption.consideration_transition_id
                        AND predecessor_consumption.consideration_lot_id=successor_consumption.consideration_lot_id
                        AND predecessor_consumption.economic_leg_identity=successor_consumption.economic_leg_identity
                        AND predecessor_consumption.amount=successor_consumption.amount
                        AND predecessor_consumption.currency=successor_consumption.currency
                    )
                ) THEN
                  RAISE EXCEPTION USING ERRCODE='23514',
                    MESSAGE='Performance Accounting correction changed liability predecessor causality';
                END IF;

                IF EXISTS (
                  SELECT 1
                  FROM public.accounting_position_origins predecessor_origin
                  WHERE predecessor_origin.tenant_id=r.tenant_id
                    AND predecessor_origin.origin_recognition_type='PERFORMANCE_ACCOUNTING_RECOGNITION'
                    AND predecessor_origin.origin_recognition_id=predecessor.id
                    AND NOT EXISTS (
                      SELECT 1
                      FROM public.accounting_position_origins successor_origin
                      WHERE successor_origin.tenant_id=r.tenant_id
                        AND successor_origin.origin_recognition_type='PERFORMANCE_ACCOUNTING_RECOGNITION'
                        AND successor_origin.origin_recognition_id=r.id
                        AND successor_origin.consideration_transition_id=predecessor_origin.consideration_transition_id
                        AND successor_origin.consideration_lot_id=predecessor_origin.consideration_lot_id
                        AND successor_origin.economic_leg_identity=predecessor_origin.economic_leg_identity
                        AND successor_origin.origin_amount=predecessor_origin.origin_amount
                        AND successor_origin.currency=predecessor_origin.currency
                        AND successor_origin.accounting_date=predecessor_origin.accounting_date
                    )
                )
                OR EXISTS (
                  SELECT 1
                  ...
                  WHERE successor_origin.tenant_id=r.tenant_id
                    ...
                    AND NOT EXISTS (
                      ...
                      WHERE predecessor_origin.tenant_id=r.tenant_id
                        AND predecessor_origin.origin_recognition_type='PERFORMANCE_ACCOUNTING_RECOGNITION'
                        AND predecessor_origin.origin_recognition_id=predecessor.id
                        AND predecessor_origin.consideration_transition_id=successor_origin.consideration_transition_id
                        AND predecessor_origin.consideration_lot_id=successor_origin.consideration_lot_id
                        AND predecessor_origin.economic_leg_identity=successor_origin.economic_leg_identity
                        AND predecessor_origin.origin_amount=successor_origin.origin_amount
                        AND predecessor_origin.currency=successor_origin.currency
                        AND predecessor_origin.accounting_date=successor_origin.accounting_date
                    )
                ) THEN
                  RAISE EXCEPTION USING ERRCODE='23514',
                    MESSAGE='Performance Accounting correction changed Contract Asset economic origin';
                END IF;
              END IF;
    ...
    private function skipIsolatedTestSchema(): bool
    {
        ...
        throw new RuntimeException(
            'Performance Accounting Recognition v1 requires the public schema.'
                  ...
                  AND correction_reference IS NULL)
                OR
    ...
    private function skipIsolatedTestSchema(): bool
    {
        ...
        throw new RuntimeException(
            'Performance Accounting Recognition v1 requires the public schema.'
                  ...
                  AND btrim(correction_reference)<>''
                  AND char_length(correction_reference)<=255)
              ),
              CHECK (
                (status='posted'
                  AND reversal_operation_id IS NULL
