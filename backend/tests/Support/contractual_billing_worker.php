    ...
    while (! is_file($path)) {
        ...
        if ((hrtime(true) - $started) / 1_000_000 > $timeoutMs) {
            throw new RuntimeException(
                "Timed out waiting for Contractual Billing barrier: {$path}",
            );
        }
    }
}
...
try {
    ...
    if ($action === 'insert_entitlement') {
        DB::table('contractual_billing_entitlements')->insert([
            ...
            'effective_at' => now(),
            'status' => 'effective',
            'recognized_by' => $payload['actor_id'],
            'recognized_at' => now(),
            'created_at' => now(),
            'updated_at' => now(),
        ]);
    ...
    throw new InvalidArgumentException(
        'Unsupported Contractual Billing concurrency worker action.',
    );
