...
final class AccountingApplicationTest extends TestCase
{
    ...
    {
        [$tenant,$actor]=$this->ready();$asset=$this->createAccount($tenant,$actor,'1100','asset','current_asset');$equity=$this->createAccount($tenant,$actor,'3100','equity','equity');
        ...
        try{app(ManageManualJournalAction::class)->post((string)$tenant->id,$draft,$actor);self::fail('Unbalanced journal posted.');}catch(AccountingValidationFailed){self::assertSame('draft',DB::table('journal_entries')->where('id',$draft)->value('status'));}
    ...
    {
        [$tenant,$actor]=$this->ready();$asset=$this->createAccount($tenant,$actor,'1250','asset','current_asset');$equity=$this->createAccount($tenant,$actor,'3250','equity','equity');
        DB::table('accounting_source_types')->insert(['origin'=>'business','key'=>'phase2_fixture','owner_module'=>'tests','description'=>'Phase 2 test fixture']);
        $source=(string)Str::ulid();$request=new BusinessPostingRequest((string)$tenant->id,(int)$actor->id,'phase2_fixture',$source,'SAR','2026-03-01','Fixture',[new JournalLineData($asset,'25','0'),new JournalLineData($equity,'0','25')]);$service=app(BusinessPostingServiceInterface::class);
        $first=DB::transaction(fn()=> $service->post($request));$second=DB::transaction(fn()=> $service->post($request));
        self::assertSame($first->journalEntryId,$second->journalEntryId);self::assertTrue($second->idempotentReplay);self::assertSame(1,DB::table('journal_entries')->where('source_id',$source)->count());
        $rolledBack=(string)Str::ulid();try{DB::transaction(function()use($service,$request,$rolledBack):void{$service->post(new BusinessPostingRequest($request->tenantId,$request->actorId,$request->sourceType,$rolledBack,'SAR',$request->entryDate,$request->description,$request->lines));throw new \RuntimeException('rollback');});}catch(\RuntimeException){}
    ...
    {
        [$tenant,$actor]=$this->ready();$asset=$this->createAccount($tenant,$actor,'1300','asset','current_asset');$equity=$this->createAccount($tenant,$actor,'3300','equity','equity');
        ...
        self::assertSame('neutralized',$projection->effect_state);self::assertSame($reversal->journalEntryId,$projection->latest_effect_journal_entry_id);
        $lines=DB::table('journal_lines')->where('journal_entry_id',$reversal->journalEntryId)->orderBy('line_number')->get();self::assertSame('0.00',(string)$lines[0]->debit);self::assertSame('50.00',(string)$lines[0]->credit);self::assertSame('opening',$lines[0]->memo);
    }

    private function actor():array{$tenant=Tenant::factory()->create(['currency'=>'SAR']);$actor=User::factory()->create(['role'=>User::ROLE_ADMINISTRATOR,'status'=>User::STATUS_ACTIVE]);TenantUser::factory()->forTenant($tenant)->forUser($actor)->active()->create();return[$tenant,$actor];}
    private function ready():array{[$tenant,$actor]=$this->actor();app(ActivateAccountingAction::class)->execute((string)$tenant->id,$actor);app(ManageAccountingPeriodAction::class)->create((string)$tenant->id,$actor,'2026-01-01','2026-12-31');return[$tenant,$actor];}
    private function account(string $code,string $type,string $class):array{return['code'=>$code,'name'=>'Account '.$code,'description'=>null,'kind'=>'posting','account_type'=>$type,'classification'=>$class,'parent_id'=>null];}
    private function createAccount(Tenant $tenant,User $actor,string $code,string $type,string $class):string{return app(ManageAccountAction::class)->create((string)$tenant->id,$actor,$this->account($code,$type,$class));}
}
